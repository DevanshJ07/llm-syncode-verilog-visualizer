 ...
 * GET /imported-experiment/{id}/prompts/{promptId}/parser-analysis (final_source).
 *
...
export async function GET(
  req: NextRequest,
  context: { params: { id: string; promptId: string } }
) {
  ...
  try {
    const res = await fetch(
      `${BACKEND_URL}/imported-experiment/${encodeURIComponent(id)}/prompts/${encodeURIComponent(promptId)}/parser-analysis?${qs.toString()}`,
      {
        method: "GET",
    ...
    return NextResponse.json(
      {
        detail: {
          ...
          message: isAbort
            ? "Imported final parser-analysis proxy timed out after 10 minutes"
