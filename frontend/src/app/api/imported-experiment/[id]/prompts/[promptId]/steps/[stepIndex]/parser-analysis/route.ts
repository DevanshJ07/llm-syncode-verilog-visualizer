/**
 * Large-body proxy for
 * GET /imported-experiment/{id}/prompts/{promptId}/steps/{stepIndex}/parser-analysis.
 *
...
export async function GET(
  req: NextRequest,
  context: { params: { id: string; promptId: string; stepIndex: string } }
) {
  ...
  try {
    const res = await fetch(
      `${BACKEND_URL}/imported-experiment/${encodeURIComponent(id)}/prompts/${encodeURIComponent(promptId)}/steps/${encodeURIComponent(stepIndex)}/parser-analysis?${qs.toString()}`,
      {
        method: "GET",
    ...
    return NextResponse.json(
      {
        detail: {
          ...
          message: isAbort
            ? "Imported step parser-analysis proxy timed out after 10 minutes"
